...
export default function ManagerClient() {
  ...
  function openEditor(product: MarketProduct, scroll = true) {
    setEditing(product);
    setEditShelf(product.shelf);
    setEditImageData("");
    setMessage(`Editing: ${product.name}`);
    if (scroll) requestAnimationFrame(() => document.getElementById("market-edit-form")?.scrollIntoView({ behavior: "smooth", block: "start" }));
  }

  async function refreshProducts(successMessage?: string) {
    const data = await api({ action: "list" });
    const nextProducts: MarketProduct[] = data.products || [];
    setProducts(nextProducts);
    if (requestedEdit) {
      const target = nextProducts.find((product) => product.id === requestedEdit || product.slug === requestedEdit);
      if (target) {
        openEditor(target, true);
        return;
      }
    }
