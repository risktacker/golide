...
export default function ResearchPage(){return <main><Header/><section className="inner-hero shell"><p className="eyebrow"><span/> GOLIDE Research</p><h1>Evidence should<br/><em>move.</em></h1><p>Research, dashboards and computational work built to move information from static reports into systems people can explore and use.</p></section><section className="research-grid shell">{work.map(({icon:Icon,...item},i)=><article className="research-card" key={item.title}><div className="research-visual"><img className="raw-fill" src={item.image+"?v=20260907"} alt={item.title} loading="lazy"/></div><div className="research-body"><div className="research-meta"><span>0{i+1} / {item.field}</span><Icon size={20}/></div><h2>{item.title}</h2><p>{item.text}</p><span className="coming-link">Case study migrating <ExternalLink size={15}/></span></div></article>)}</section><section className="research-note shell"><span>Research direction</span><p>Emerging infectious diseases, structural biology and computational surveillance—connected by one question: how can better information produce earlier, smarter action?</p></section><Footer/></main>}
